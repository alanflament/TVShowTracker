//
//  DefaultDataImportUseCase.swift
//  TVShowTracker
//
...
final class DefaultDataImportUseCase: DataImportUseCase {
    ...
    func importBackup(
        ...
        // Reload even when a later write fails after earlier records were restored.
        defer { didImport() }
        for item in media {
        ...
        await withTaskGroup(of: Enrichment.self) { group in
            ...
            while let enrichment = await group.next() {
                ...
                if let seasons = enrichment.seasons,
                   let currentItem = followedMediaStore.item(id: enrichment.item.id) {
