...
struct CalendarEpisodeCardView: View {
    ...
    var body: some View {
        HStack(alignment: .center, spacing: 12) {
            MediaPosterView(url: episode.posterURL, kind: episode.candidate.kind, width: 80, height: 120)
            ...
            VStack(alignment: .leading, spacing: 8) {
                HStack(spacing: 8) {
                    ...
                    if episode.additionalAvailableEpisodeCount > 0 {
                        AdditionalEpisodesBadgeView(count: episode.additionalAvailableEpisodeCount)
                    }
                }
                HStack(spacing: 7) {
